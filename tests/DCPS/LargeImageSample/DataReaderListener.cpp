...
{
  try {
    Messenger::ImageDataReader_var message_dr =
      Messenger::ImageDataReader::_narrow(reader);
    ...
    Messenger::ImageSeq images;
    DDS::SampleInfoSeq info;

    DDS::ReturnCode_t error = message_dr->take(images,
                                               info,
    ...
    if (error == DDS::RETCODE_OK) {
      ...
      for (unsigned int i = 0; i < images.length(); ++i) {
        ...
        if (si.valid_data) {
          const Messenger::Image& message = images[i];
